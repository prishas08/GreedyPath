...
public class DrawPath
{
	...
	{
		final String file = "mona-20k2.txt"; //change to try other files
...
//		// Read in the number of points in the path
		...
		double lastX = p.getAPoint(0).getX(); //first points
		double lastY = p.getAPoint(0).getY();
...
//
		...
		for (int i = 1; i < N; i++)
		{
			double x = p.getAPoint(i).getX();
			double y = p.getAPoint(i).getY();
